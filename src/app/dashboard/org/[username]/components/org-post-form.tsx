...
import { OrgPostFields } from "./org-post";
...
const AddOrgPostMutation = graphql(
  `
    mutation AddOrgPost(
      $orgId: String!
      $content: String!
      ...
      addOrgPost(orgId: $orgId, content: $content, imgUrl: $imgUrl, sdg: $sdg) {
        id
        ...OrgPostFields
      }
    }
...
export function OrgPostForm({ orgId }: { orgId: string }) {
  const [{ fetching }, addPost] = useMutation(AddOrgPostMutation);
  ...
  function onSubmit(data: z.infer<typeof FormSchema>) {
    addPost({ orgId, content: data.content, imgUrl, sdg: selectedSdg }).then(
      (res) => {
        if (res.error) {
