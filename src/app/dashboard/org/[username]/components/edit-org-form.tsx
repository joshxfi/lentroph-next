...
import { OrgBannerFields } from "./org-banner";
import { OrgSidebarFields } from "./org-sidebar";
import {
  Sheet,
  SheetContent,
  SheetDescription,
...
const EditOrgMutation = graphql(
  `
    mutation EditOrg($id: String!, $input: AddOrgInput!) {
  ...
  `,
  [OrgBannerFields, OrgSidebarFields, OrgFields],
);

export function EditOrgForm({ org }: { org: FragmentOf<typeof OrgFields> }) {
  const orgData = readFragment(OrgFields, org);

  const [{ fetching }, editOrgFn] = useMutation(EditOrgMutation);
  const [imgUrl, setImgUrl] = useState(orgData.image);
